...
- `WORKER_MAX_UPTIME_HOURS` (worker, default `0`; uptime threshold in hours)
- `WORKER_MAX_UPTIME_RECYCLE_ENABLED` (worker, default `false`; must be `true` to allow auto-exit/recycle when max uptime is reached)
...
- `SYMBOL_SESSION_MODE_XAUUSD` (worker; `auto|always_open|always_closed|fx_24_5|ifc_metal`)
- `SYMBOL_SESSION_MODE_THBUSD` (worker; `auto|always_open|always_closed|fx_24_5|ifc_metal`)
- `SYMBOL_SESSION_MODE_BTCUSD` (worker; `auto|always_open|always_closed|fx_24_5|ifc_metal`)
